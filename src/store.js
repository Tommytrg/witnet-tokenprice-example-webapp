    ...
    async fetchPolls ({ state, commit, dispatch }) {
      ...
      if (state.contractInstance) {
        ...
        for (let i = lastDay; i >= 0 && i >= lastDay - ITEMS_PER_PAGE; i--) {
          ...
          const status = pollStatesMap[await contract.methods.getDayState(i).call()]
          const startDate = new Date(parseInt(state.firstDayTimestamp) * 1000 + parseInt(state.contestPeriod) * i)
          const endDate = new Date(parseInt(state.firstDayTimestamp) * 1000 + parseInt(state.contestPeriod) * (i + 1))
